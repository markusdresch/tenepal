...
RUN_DIR="$OUT_ROOT/runs_local"
SCORE_FILE="$OUT_ROOT/scoreboard_local.tsv"
...
run_profile() {
  ...
  base=(venv/bin/python -m tenepal process "$clip" -o "$out")

  case "$profile" in
    w_only_orth)
      base=(venv/bin/python -m tenepal --whisper-only --spanish-orthography process "$clip" -o "$out")
      ;;
    w_only)
      base=(venv/bin/python -m tenepal --whisper-only process "$clip" -o "$out")
      ;;
    w_only_orth_rescue)
      base=(venv/bin/python -m tenepal --whisper-only --spanish-orthography --whisper-rescue process "$clip" -o "$out")
      ;;
    w_only_rescue)
      base=(venv/bin/python -m tenepal --whisper-only --whisper-rescue process "$clip" -o "$out")
      ;;
    hybrid_conf_lo)
      base=(venv/bin/python -m tenepal --confidence -0.7 process "$clip" -o "$out")
      ;;
    allosaurus_only)
      base=(venv/bin/python -m tenepal --backend allosaurus "$clip" -o "$out")
      ;;
    *)
  ...
  echo "[run] $id :: $profile"
  "${base[@]}" >/tmp/edge_local_${id}_${profile}.log 2>&1 || {
    echo "[run] failed $id :: $profile (see /tmp/edge_local_${id}_${profile}.log)"
    return 1
  }
}
...
run_all() {
  profiles=(w_only_orth w_only w_only_orth_rescue w_only_rescue hybrid_conf_lo allosaurus_only)
  ...
  echo
  echo "=== LOCAL EDGE DEBUG SCOREBOARD ==="
